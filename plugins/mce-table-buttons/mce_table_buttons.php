...
class MCE_Table_Buttons {
	...
	public static function mce_external_plugins( $plugin_array ) {
		$plugin_url = THEME_URL . '/plugins/mce-table-buttons/';
		global $tinymce_version;
		$variant = ( defined('SCRIPT_DEBUG') && SCRIPT_DEBUG ) ? '' : '.min';

		if ( version_compare( $tinymce_version, '400', '<' ) ) {

			wp_register_style( 'mce-table-buttons', $plugin_url . 'tinymce3-assets/mce-table-buttons.css' );
			wp_print_styles( 'mce-table-buttons' );

			$plugin_array['table'] = $plugin_url . 'tinymce3-table/editor_plugin.js';
			$plugin_array['mcetablebuttons'] = $plugin_url . 'tinymce3-assets/mce-table-buttons.js';

		} elseif ( version_compare( $tinymce_version, '4100', '<' ) ) {

			$plugin_array['table'] = $plugin_url . 'tinymce4-table/plugin' . $variant . '.js';

		} else {

			$plugin_array['table'] = $plugin_url . 'tinymce41-table/plugin' . $variant . '.js';

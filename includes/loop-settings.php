...
<div class="fl-custom-query fl-loop-data-source" data-source="custom_query">
    <div id="fl-builder-settings-section-general" class="fl-builder-settings-section">
        ...
        <p style="padding:20px 10px 10px 30px !important">On initial edit, after adding the site details...save, close and reopen this module to see the filters.</p>
        <table class="fl-form-table">
        <?php
        
            //@todo change this into one drop down
            $cur_id =  get_current_blog_id();
            $cur_url = home_url();
            // Data Source
            FLBuilder::render_settings_field('source_site_id', array(
                'type'          => 'text',
                'label'         => __( 'Source Site ID', 'fl-builder' ),
                'default'       => $cur_id,
            ), $settings);
             FLBuilder::render_settings_field('source_site_home', array(
                'type'          => 'text',
                'label'         => __( 'Source Site URL', 'fl-builder' ),
                'default'       =>  $cur_url,
            ), $settings);


             //@todo restrict to objects
                $siteurl = $settings->source_site_home != "" ? $settings->source_site_home :home_url();
                $response = wp_remote_get( $siteurl . "/wp-json/wp/v2/types", array('timeout' => 10000) );
    ...
    <div id="fl-builder-settings-section-filter" class="fl-builder-settings-section">
        ...
        //Taxonomy filter
             $siteurl = $settings->source_site_home != "" ? $settings->source_site_home :home_url();
            $response = wp_remote_get( $siteurl . "/wp-json/wp/v2/categories?per_page=100", array('timeout' => 10000) );
            ...
            if ( 200 != $response_code && ! empty( $response_message ) ) {
                ...
                $categories = json_decode(wp_remote_retrieve_body( $response ));
                    ...
                    if( ! empty( $categories ) ) {
                     ...
                     //@todo ajax refresh
                     FLBuilder::render_settings_field('tax_query', array(
                        'type'          => 'select',
